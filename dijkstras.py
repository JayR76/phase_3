...
def dijkstras(graph, source, dest):
    distances = {}
    for vertex in graph:
        distances[vertex] = 999999       #finding the shortest path from a source to particular destination
    distances[source] = 0
    heap = []
    ...
    node = dest
    while node != source:
        path.insert(0, node)  #append(0,node) want in reverse path we use append method
        node = previous[node]
